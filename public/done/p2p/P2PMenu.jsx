...
const P2PMenu = () => {
    ...
    const handleAdClick = (ad) => {
        ...
        if (ad.type === 'buy') {
            navigate(`/p2p/buy/${ad.id}`, { state: { listing: ad } });
        } else {
            navigate(`/p2p/sell/${ad.id}`, { state: { listing: ad } });
        }
    };
    
    const handleDealClick = (deal) => {
        // Перенаправляем на страницу сделки с передачей данных через state
        navigate(`/p2p/deals`, { state: { selectedDeal: deal } });
    };
                                ...
                                >
                                    <div>
                                        <div className="flex items-center">
                                            ...
                                            <span className="font-medium">{ad.asset}</span>
                                        </div>
                                        <div className="text-sm text-gray-400 mt-1">
                                            {ad.price} RUB • {ad.amount} {ad.asset}
                                        </div>
                                    </div>
