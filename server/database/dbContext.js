import { pgClient, redisClient } from './db.js';
...
async function getUserBalance(userId, coinId) {
    ...
    const result = await pgClient.query(query, [userId, coinId]);
...
async function addTransaction(userId, coinId, amount, type) {
    ...
    const result = await pgClient.query(query, [userId, coinId, amount, type]);
...
async function updateUserBalance(userId, coinId, newBalance) {
    ...
    const result = await pgClient.query(query, [newBalance, userId, coinId]);
...
export { getUserById, getUserByTelegramId, addUser, getAllCurrencies, getAllCoins, getUserBalance, addTransaction, updateUserBalance, getUserTransactions, getAllExchangeRates, getAllCommissions };
